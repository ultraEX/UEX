...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/uexd.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/uex-cli.1
Source22:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/uex-qt.1

#selinux
Source30:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/rpm/bitcoin.te
# Source31 - what about uex-tx and bench_uex ???
...
The uex-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the uex-tx utility allows you to create a custom transaction, and
the bench_uex utility can be used to perform some benchmarks.
...
mv %{buildroot}%{_bindir}/uexd %{buildroot}%{_sbindir}/uexd

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/uexd 0750 bitcoin bitcoin -
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/uexd -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/uexd.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/uex-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/uex-qt.1
%endif
...
%attr(0755,root,root) %{_bindir}/uex-qt
...
%attr(0644,root,root) %{_mandir}/man1/uex-qt.1*
%endif
...
%attr(0755,root,root) %{_sbindir}/uexd
...
%attr(0644,root,root) %{_mandir}/man1/uexd.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/uex-cli
%attr(0755,root,root) %{_bindir}/uex-tx
%attr(0755,root,root) %{_bindir}/bench_uex
%attr(0644,root,root) %{_mandir}/man1/uex-cli.1*
