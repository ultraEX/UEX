...
UniValue getloosechange(const JSONRPCRequest& request)
{
	CWallet * const pwallet = GetWalletForJSONRPCRequest(request);

    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "getloosechange [{\"txid\":\"id\",\"vout\":n,\"amount\":n},...] divtimes ( locktime ) ( replaceable )\n"
            "\nCreate a transaction spending the given inputs and creating new loose change outputs.\n"
            ...
            "         \"amount\":n,       (numeric, required) The output amount\n"
            "         \"sequence\":n      (numeric, optional) The sequence number\n"
            "       } \n"
            "       ,...\n"
            "     ]\n"
            "2. divtimes                  (numeric, required) The divide times\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("getloosechange", "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0, \\\"amount\\\":1.0, }]\" divtimes")
        );		

    RPCTypeCheck(request.params, {UniValue::VARR, UniValue::VNUM, UniValue::VNUM, UniValue::VBOOL}, true);
    if (request.params[0].isNull() || request.params[1].isNull() )
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, arguments 1 and 2 must be non-null");

    UniValue inputs = request.params[0].get_array();
    int64_t  divtimes = request.params[1].get_int64();
	if (divtimes <= 1 || divtimes > std::numeric_limits<uint32_t>::max())
		throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, divtimes out of range");
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
		...
		const UniValue& vamount_v = find_value(o, "amount");
	...
	// need optimize: use fix fee
	int64_t	useFixFee = 0.001 * COIN;
	if (amount <= useFixFee)
		throw JSONRPCError(RPC_INVALID_PARAMETER, "balance little, less than the txfee");
	amount -= useFixFee; 

	std::set<CTxDestination> destinations;
    for (int64_t dt=0; dt<divtimes; dt++) {
		...
		pwallet->LearnRelatedScripts(newKey, output_type);
		CTxDestination destination = GetDestinationForKey(newKey, output_type);
		if (!IsValidDestination(destination)) {
			throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Bitcoin address: "));
		}

		if (!destinations.insert(destination).second) {
			throw JSONRPCError(RPC_INVALID_PARAMETER, std::string("Invalid parameter, duplicated address: "));
		}

		CScript scriptPubKey = GetScriptForDestination(destination);
		CAmount nAmount = amount/divtimes;
    ...
    std::string hexTx = EncodeHexTx(rawTx); 
    // signrawtransaction: Make a JSONRPCRequest to pass on to the right signrawtransaction* command
    JSONRPCRequest new_request;
    new_request.id = request.id;
    new_request.params.setArray();

    // sign with the wallet which does not take a privkeys parameter
    new_request.params.push_back(hexTx);
    UniValue signTx = signrawtransactionwithwallet(new_request);

    // sendrawtransaction:
    JSONRPCRequest send_request;
    send_request.id = request.id; 
    send_request.params.setArray();
    UniValue hexSignTx = find_value(signTx, "hex");
    send_request.params.push_back(hexSignTx.getValStr());
    
    return sendrawtransaction(send_request);
}

UniValue loosewallet(const JSONRPCRequest& request)
{
	CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
	
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw std::runtime_error(
            "loosewallet divtimes ( locktime ) ( replaceable )\n"
            "\nCreate a transaction spending the unspent list inputs and creating new loose change outputs.\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.\n"

            "\nArguments:\n"
            "1. divtimes                  (numeric, required) The divide times\n"
            "2. locktime                  (numeric, optional, default=0) Raw locktime. Non-0 value also locktime-activates inputs\n"
            "3. replaceable               (boolean, optional, default=false) Marks this transaction as BIP125 replaceable.\n"
            "                             Allows this transaction to be replaced by a transaction with higher fees. If provided, it is an error if explicit sequence numbers are incompatible.\n"
            "\nResult:\n"
            "\"transaction\"              (string) hex string of the transaction\n"

            "\nExamples:\n"
            + HelpExampleCli("loosewallet", "divtimes")
        );

    RPCTypeCheck(request.params, {UniValue::VNUM, UniValue::VNUM, UniValue::VBOOL}, true);
    if (request.params[0].isNull())
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, arguments 1 must be non-null");

    
    int64_t  divtimes = request.params[0].get_int64();
	if (divtimes < 0 || divtimes > std::numeric_limits<uint32_t>::max())
		throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, divtimes out of range");
    ...
    bool rbfOptIn = request.params[2].isTrue();
	
	// list unspent as the raw transaction input
    ObserveSafeMode();
    ...
    for (const COutput& out : vecOutputs) {		
		UniValue	tx_;
		tx_.pushKV("txid", out.tx->GetHash().GetHex());
		tx_.pushKV("vout", out.i);
		listunspent.push_back(tx_);
		amount += out.tx->tx->vout[out.i].nValue;
    }

    for (unsigned int idx = 0; idx < listunspent.size(); idx++) {
        const UniValue& input = listunspent[idx];
        const UniValue& o = input.get_obj();
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart uexd with -deprecatedrpc=addwitnessaddress.\n"
...
{ //  category              name                                actor (function)                argNames
    ...
    { "wallet",             "getloosechange",                   &getloosechange,                {"inputs","divtimes","locktime","replaceable"} },
    { "wallet",             "loosewallet",                      &loosewallet,                   {"divtimes","locktime","replaceable"} },
